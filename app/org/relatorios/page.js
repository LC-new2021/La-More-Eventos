...
const maskCPF = (cpf) => {
  ...
  return cpf.length > 5 ? `${cpf.slice(0, 3)}***${cpf.slice(-2)}` : "***";
};
...
export default function OrgRelatoriosPage() {
  ...
  // EXPORTAÇÕES EXCLUSIVAS POR TEMA COM PROTEÇÃO LGPD
  ...
  // 2. Exportação EXCLUSIVA de Cortesias por Pessoa (Excel) com máscara LGPD
  async function exportarCortesiasXLSX() {
    ...
    wsPessoas.getRow(4).values = ["Cliente (Pessoa)", "Código Cartão", "CPF (Mascarado)", "Operador Principal", "Qtd Recargas", "Total Concedido (R$)", "Total Consumido (R$)", "Total Devolvido (R$)", "Saldo Restante (R$)"];
          ...
          <div className="bg-white p-5 rounded-3xl border-2 border-gray-100 shadow-sm flex flex-col md:flex-row items-center justify-between gap-4">
            <div className="w-full md:w-auto flex-1">
              <input
                ...
                placeholder="🔍 Filtrar cortesia por nome da pessoa ou código do cartão..."
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-blue-200 text-xs font-semibold mt-1">
                          Responsável: <b>{pessoa.operadorPrincipal}</b> • CPF: <b>{maskCPF(pessoa.clienteCpf)}</b> {pessoa.clienteCelular && `• Tel: ${maskPhone(pessoa.clienteCelular)}`}
                        </p>
                      </div>
                    </div>
