...
export async function criarPixMercadoPago({ token, valor, descricao, codigoPedido, payer, appUrl }) {
  ...
  const payload = {
    ...
    payer: {
      email: payer.email || 'contato@lamorebilheteria.com.br',
  ...
  // Suporte a CNPJ (14 dígitos) ou CPF (11 dígitos)
  if (docLimpo.length === 14 && !/^(\d)\1{13}$/.test(docLimpo)) {
    payload.payer.identification = {
      type: 'CNPJ',
      number: docLimpo,
    };
  } else if (docLimpo.length === 11 && !/^(\d)\1{10}$/.test(docLimpo)) {
    payload.payer.identification = {
      type: 'CPF',
      number: docLimpo,
    };
