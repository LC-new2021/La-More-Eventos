...
export async function generateGoogleWalletPassUrl(cartao, cliente) {
  ...
  if (!issuerIdRaw || (!credentialsEnv && !credentialsPath)) {
  ...
  const classId = `${issuerId}.lamore_consumo_digital`;
  ...
  try {
    ...
    const newObject = {
      ...
      logo: {
        sourceUri: {
          uri: `${process.env.NEXT_PUBLIC_URL || 'https://eventos.grupolamore.com.br'}/logo.png`
        }
      },
      cardTitle: {
        defaultValue: {
    ...
    const claims = {
      ...
      aud: 'google',
      typ: 'savetowallet',
      origins: [process.env.NEXT_PUBLIC_URL || 'https://eventos.grupolamore.com.br'],
      payload: {
